...
class DefaultController extends Controller {
    ...
    public function actionCreate() {
        if (Yii::$app->request->post()) {
            ...
            echo json_encode($persona->id_empleado);
        } else {
    ...
    public function actionUpdate() {
        if (Yii::$app->request->post()) {
            ...
            try {
                ...
                $persona = Empleado::findOne($post['id_empleado']);
                $persona->dni = $post['dni'];
                $persona->nombres = $post['nombres'];
                $persona->apellido_paterno = $post['apellido_paterno'];
                $persona->apellido_materno = $post['apellido_materno'];
...
//        $length = ($perpage * $page) - 1;
        ...
        try {
            $command = Yii::$app->db->createCommand('call listadoEmpleado(:row,:length,:buscar)');
        ...
        foreach ($result as $k => $row) {
            $data[] = [
                ...
                "accion" => '<button class="btn btn-sm btn-light-success font-weight-bold mr-2" onclick="funcionEditar(' . $row["id_empleado"] . ')"><i class="flaticon-edit"></i>Editar</button>
                             <button class="btn btn-sm btn-light-danger font-weight-bold mr-2" onclick="funcionEliminar(' . $row["id_empleado"] . ')"><i class="flaticon-delete"></i>Eliminar</button>',
            ];
        }
